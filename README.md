...
Ten tests verify the archive, running the server from the **extracted bundle** rather than the repo — the only way to catch a missing dependency, since the repo has every devDependency installed alongside. One starts the entry point directly and asserts on its captured stderr, so diagnosing a startup failure never requires the shell. Another drives a real JSON-RPC handshake and then asserts the process is **still alive** — and still answering after idling — because "it started" is not the property a client depends on, and the regression that prompted these tests started perfectly well before exiting. They skip with instructions when `build/frontier.mcpb` is absent.

They also **refuse to run against a stale bundle**. `npm run bundle` writes `build/frontier.mcpb.stamp.json` holding a SHA-256 over everything staged from the repo — `dist/`, the manifest, `package.json` — and each test checks that stamp against a freshly computed hash before touching the archive. A mismatch fails immediately with `STALE BUNDLE — run: npm run bundle` and both digests, instead of letting the suite report `MODULE_NOT_FOUND` and missing tools against an entry point that no longer exists. That distinction was not free: an outdated artifact once produced six failures that were indistinguishable from a broken build, and five of the six were misdiagnosed before the artifact was suspected. The hash is over content rather than mtime because `pretest` runs `tsc`, which rewrites `dist/` on every `npm test` — an mtime comparison called the bundle stale on every run and was worth nothing.
